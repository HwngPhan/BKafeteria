...
test.describe("Dashboard Page", () => {
  ...
  test("should show Explore Vendors and View Menu CTA links", async ({
    page,
  }) => {
    await expect(
      page.getByRole("link", { name: /Khám phá cửa hàng|Explore Vendors/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /Xem thực đơn|View Menu/i }),
    ).toBeVisible();
  });

  test("should navigate to /vendors from the CTA link", async ({ page }) => {
    await page
      .getByRole("link", { name: /Khám phá cửa hàng|Explore Vendors/i })
      .click();
