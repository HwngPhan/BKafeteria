...
export default function StaffOrderDetailPage() {
  ...
  const vendorOrderId = searchParams.get("v") ?? "";
  const { data: orderData, isLoading } = useOrderById(params.id as string);
  const vendorOrder = orderData?.vendorOrders.find(
    (vo) => vo.vendorOrderId === vendorOrderId
  );
  ...
  const handleConfirm = async () => {
    ...
    try {
      await confirmOrder.mutateAsync(vendorOrder.vendorOrderId);
  ...
  const isCanceled = vendorOrder.status === "CANCELED";
  ...
  const currentStatusIndex = getStatusIndex(vendorOrder.status);
  const total = (vendorOrder.orderItems || []).reduce(
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 flex items-center gap-2">
                    ...
                    {orderData.customerId.substring(0, 8)}…
                  </p>
                </div>
              </div>
              <Badge
