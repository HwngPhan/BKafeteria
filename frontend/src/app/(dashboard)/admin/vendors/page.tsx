...
export default function AdminVendorsPage() {
  ...
  const handleApprove = (id: string, name: string) => {
    approveVendor.mutate(id, {
      onSuccess: () =>
        toast.success(
          t("admin.vendors.toast_approved").replace("{name}", name),
        ),
    });
  };
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <Button
          variant="outline"
          className="rounded-2xl h-12 gap-2 border-none bg-white shadow-lg shadow-secondary/5 font-bold"
        >
          <Filter size={18} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredVendors && filteredVendors.length > 0 ? (
          filteredVendors.map((vendor) => (
            <Card
              key={vendor.vendorId}
              className="rounded-[2.5rem] border-none shadow-2xl shadow-secondary/5 overflow-hidden bg-white group hover:shadow-primary/5 transition-all duration-500"
            >
              <CardHeader className="p-8 pb-4">
                <div className="flex justify-between items-start">
                  ...
                  <Badge
                    className={cn(
                      "rounded-full px-4 py-1 text-[10px] font-bold border-none shadow-sm",
                ...
                {vendor.status === "PENDING" && (
                  <div className="pt-2 flex gap-3">
                    <Button
                      ...
                      onClick={() =>
                        handleApprove(vendor.vendorId, vendor.name)
                      }
