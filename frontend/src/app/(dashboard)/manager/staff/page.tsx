...
import { useAllUsers, useAssignVendor } from '@/features/user/data-access/user.queries'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Input } from '@/components/ui/input'
import { Loader2, UserPlus, Search, Mail, UserX, CheckCircle2, Shield, BadgeCheck, MoreVertical } from 'lucide-react'
...
export default function ManagerStaffPage() {
  ...
  const handleAddStaff = (e: React.FormEvent) => {
    e.preventDefault()
    if (!vendor) return
    assignVendor.mutate({ vendorId: vendor.vendorId, email }, {
      onSuccess: () => {
        toast.success(t('manager.staff.toast_added').replace('{email}', email))
  ...
  const handleRemoveStaff = (user: any) => {
    if (confirm(t('manager.staff.confirm_remove').replace('{name}', user.fullName))) {
                ...
                {staffMembers && staffMembers.length > 0 ? (
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent className="rounded-xl border-none shadow-xl">
                          <DropdownMenuItem className="rounded-lg text-red-500 focus:text-red-500 cursor-pointer" onClick={() => handleRemoveStaff(staff)}>
                            <UserX size={16} className="mr-2" /> {t('manager.staff.remove')}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
                    ...
                    <div className="space-y-2">
                      <h3 className="text-xl font-bold text-primary">{t('manager.staff.empty')}</h3>
                      <p className="text-muted-foreground max-w-xs">{t('manager.staff.empty_desc')}</p>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="space-y-8">
          <Card className="rounded-[2.5rem] border-none shadow-xl shadow-secondary/5 overflow-hidden bg-white">
            <CardHeader className="p-8 border-b bg-secondary/5">
              <CardTitle className="text-lg font-bold">{t('manager.staff.note_title')}</CardTitle>
            </CardHeader>
            <CardContent className="p-8 space-y-4">
              {[t('manager.staff.note_1'), t('manager.staff.note_2')].map((note, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <p className="text-xs font-medium text-muted-foreground leading-relaxed">{note}</p>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-xl shadow-secondary/5 overflow-hidden bg-gradient-to-br from-secondary to-primary text-primary-foreground">
            <CardContent className="p-8 space-y-4">
              ...
              <p className="text-sm opacity-80 leading-relaxed font-medium">{t('manager.staff.support_desc')}</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden bg-white max-w-md">
          <DialogHeader className="p-8 bg-secondary/5 border-b">
            <DialogTitle className="text-xl font-black text-primary">{t('manager.staff.dialog_title')}</DialogTitle>
            <DialogDescription className="font-medium text-xs">{t('manager.staff.dialog_desc')}</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAddStaff}>
            <div className="p-8 space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-bold text-primary uppercase tracking-wider ml-1">{t('manager.staff.email_label')}</label>
            ...
            <DialogFooter className="p-8 pt-0 flex gap-3">
              <Button type="button" variant="ghost" onClick={() => setIsDialogOpen(false)} className="rounded-2xl h-12 px-6 font-bold flex-1">
                {t('manager.staff.cancel')}
              </Button>
              <Button type="submit" disabled={assignVendor.isPending} className="rounded-2xl h-12 px-8 font-bold flex-1 gap-2 shadow-lg shadow-primary/20">
                {assignVendor.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserPlus size={18} />}
