...
export const GetMyOrdersApi = async (): Promise<OrderDto[]> => {
  const response = await fetchWithToken(TokenType.authToken, `${BASE_URL}/get-my-order`, {
    method: 'GET',
  });
...
export const PayOrderApi = async (id: string): Promise<OrderDto> => {
